        ...
        for evaluator in evals:
            try:
                ...
                if isinstance(eval_log, tuple):
                    ...
                    logger.info(
                        f"Received single EvalLog from {evaluator.get_name()} on {model_name}"
                    )
                ...
                if success:
                    for log in logs:
                        ...
                        for metric_key, metric_object in log.results.scores[0].metrics.items():
                            metric_name = (
                                f"{evaluator.get_name()}_{metric_key}_{metric_object.name}"
                            )
...
def evaluate(
    ...
    for i, checkpoint_info in enumerate(checkpoints):
        ...
        pd.DataFrame([checkpoint_results]).to_csv(output_file, mode="a", header=False, index=False)
